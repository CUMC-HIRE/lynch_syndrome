...
#    print(t_matrix)
    ...
    for t in time:
        ...
        if t == MCLIR_age:
            Distribution[names[run_spec.guidelines]] = Distribution[names[run_spec.guidelines]] + sum(Distribution[4:])
...
def CRC_MCLIR_results_by_interval(genes = ps.genes, genders = ps.genders, 
                            intervals = ps.intervals, csy_start_age = ps.START_AGE,
                            overwrite_file = False):
    for gender in genders:
        this_gender = g.gender_obj(gender)
...
#                 fontsize = 14, y = 0.94)
    ...
    for gene in genes:
        ...
        ax_array[i].set_ylim(bottom = 0, top = .14)
...
#                 fontsize = 14, y = 0.94)
    ...
    for gene in genes:
        ...
        ymax = cancer_df[least_agro].max() + 0.01
            #file_name = 'CRC_incidence_'+ gene + '_'+ 'interval_constant_'+ str(interval) + '_combined_gender.png'
        ax_array[i].set_ylim(bottom = 0, top = .15)
        ax_array[i].set_xlim(left = 25, right = 75)
        i += 1
...
def CD_MCLIR_natural_history(MCLIR_age):
    ...
    for gene in genes:
        ...
        ax_array[i].set_xlim(left = 25, right = 75)
        i += 1
    plt.savefig(ps.cd_results/'optimal_strats_' + str(MCLIR_age) + '_perf_intervention.png', bbox_inches = 'tight', dpi = 500)
...
#                 fontsize = 14, y = 0.94)
    ...
    for gene in genes:
        ...
        ymax = cancer_df[least_agro].max() + 0.01
            #file_name = 'CRC_incidence_'+ gene + '_'+ 'interval_constant_'+ str(interval) + '_combined_gender.png'
        ax_array[i].set_ylim(bottom = 0, top = .55)
        ax_array[i].set_xlim(left = 25, right = 75)
        i += 1
...
#                 fontsize = 14, y = 0.94)
    ...
    for gene in genes:
        ...
        ymax = cancer_df[least_agro].max() + 0.01
            #file_name = 'CRC_incidence_'+ gene + '_'+ 'interval_constant_'+ str(interval) + '_combined_gender.png'
        ax_array[i].set_ylim(bottom = 0, top = .6)
        ax_array[i].set_xlim(left = 25, right = 75)
        i += 1
...
#                 fontsize = 14, y = 0.94)
    ...
    for gene in genes:
        ...
        for col in cols:
            ax_array[i].plot(ps.age_time, cancer_df_m[col], 'g', label = "Perfect Intervention")
            ax_array[i].plot(ps.age_time, cancer_df[col], "--", color="grey", label = "Background Rate")
        ax_array[i].legend()
        ymax = cancer_df[least_agro].max() + 0.01
            #file_name = 'CRC_incidence_'+ gene + '_'+ 'interval_constant_'+ str(interval) + '_combined_gender.png'
        ax_array[i].set_ylim(bottom = 0, top = .6)
...
#                 fontsize = 14, y = 0.94)
    ...
    for gene in genes:
        ...
        for col in cols:
            ax_array[i].plot(ps.age_time, (cancer_df[col] - cancer_df_m[col])/cancer_df[col] * 100, 'b', label = "MCLIR")
        ax_array[i].legend()
        ymax = 100
            #file_name = 'CRC_incidence_'+ gene + '_'+ 'interval_constant_'+ str(interval) + '_combined_gender.png'
        ax_array[i].set_ylim(bottom = 0, top = ymax)
